...
ggheat=function(m, rescaling='none', clustering='none', labCol=T, labRow=T, border=FALSE, heatscale= c(low='blue',high='red'))
{
  ...
  #=g2+theme(panel.grid.minor=element_line(colour=NA), panel.grid.major=element_line(colour=NA),
...
## data(mtcars)
## x= as.matrix(mtcars)
## ggheat(x, clustCol=T)+ opts(panel.background=element_rect(fill='pink'))


data(mtcars)
x=as.matrix(mtcars)
ggheat(t(promoter.pc), heatscale=c(low='red', high='yellow'))
