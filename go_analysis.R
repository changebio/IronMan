...
ego<- lapply(1:7,function(i,x)enrichGO(gene = x[[i]]$geneId,organism = "human",ont = "CC",pvalueCutoff = 0.01,pAdjustMethod = "BH",qvalueCutoff = 0.05,readable = TRUE),x = split(k562.pk.dnase,k562.pk.dnase$State))
...
a<- Reduce(rbind,lapply(ego,function(x)x@result))
a$group<- rep(names(ego),as.numeric(sapply(ego,function(x)nrow(x@result))))
